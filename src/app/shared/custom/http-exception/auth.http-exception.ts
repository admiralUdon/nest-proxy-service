...
export type AuthHttpExceptionType = {
...
export class AuthHttpException extends HttpException {
    ...
    constructor(stack: AuthHttpExceptionType) {
        const timestamp = ISO8601DateTime(new Date());
        try {
            if (stack instanceof AuthHttpException) {   
                const error = stack?.getResponse() as AuthHttpExceptionType;
